 ...
 */
class ctGoogleFontControl extends WP_Customize_Control implements ctControlsFilterableInterface {
	private $fonts = false;
	...
	public function __construct( $manager, $id, $options = array() ) {
		$this->amount         = isset( $options['amount'] ) ? $options['amount'] : 'all';
		$this->fonts    = self::get_fonts( $this->amount );
	...
	public function render_content() {
		...
		if ( ! empty( $this->fonts ) ) {
			...
			<label>
				...
				<select id="customizer_font_<?php echo esc_attr($this->selectId) ?>" <?php $this->link(); ?> data-default-value="<?php echo esc_attr($this->setting->default)?>">
					<?php
					foreach ( $this->fonts as $k => $v ) {
						printf( '<option class="%s" value="%s" %s>%s</option>',
							join( ' ', $v->subsets ),
							$v->family,
		...
		if ( file_exists( $fontFile ) && time() - $cachetime < filemtime( $fontFile ) ) {
			...
			if ( ! file_exists( $cacheDir ) ) {
				...
				if ( mkdir( $cacheDir ) ) {
					...
					//WP_Filesystem();
					//global  $wp_filesystem;
					//$wp_filesystem->put_contents($fontContent,$fontContent['body']);
					file_put_contents($fontContent,$fontContent['body']);
				}
			}
