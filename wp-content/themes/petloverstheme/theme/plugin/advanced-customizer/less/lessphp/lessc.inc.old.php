 ...
 * by `lessc_parser_old` into a syntax tree, then it is compiled into another tree
 ...
 * time.
 *
 * The `lessc_parser_old` class is only concerned with parsing its input.
 *
...
class lessc_old {
	...
	protected function injectVariables($args) {
		$this->pushEnv();
		$parser = new lessc_parser_old($this, __METHOD__);
	...
	protected function makeParser($name) {
		$parser = new lessc_parser_old($this, $name);
	...
	protected function newFormatter() {
		$className = "lessc_formatter_lessjs_old";
...
class lessc_parser_old {
	...
	public function __construct($lessc, $sourceName = null) {
		...
		if (!self::$operatorString) {
			self::$operatorString =
				'('.implode('|', array_map(array('lessc_old', 'preg_quote'),
					array_keys(self::$precedence))).')';

			$commentSingle = lessc_old::preg_quote(self::$commentSingle);
			$commentMultiLeft = lessc_old::preg_quote(self::$commentMultiLeft);
			$commentMultiRight = lessc_old::preg_quote(self::$commentMultiRight);
	 ...
	 *
	 * Consider the function lessc_old::keyword(). (all parse functions are
	 * structured the same)
	 *
	 ...
	 *
	 * All of these parse functions are powered by lessc_old::match(), which behaves
	 ...
	 * avoid this, lessc_old::seek() is used to remember and set buffer positions.
	 *
	...
	protected function isDirective($dirname, $directives) {
		...
		$pattern = implode("|",
			array_map(array("lessc_old", "preg_quote"), $directives));
	...
	protected function expressionList(&$exps) {
		...
		$exps = lessc_old::compressList($values, ' ');
		return true;
	}
	...
	public function propertyValue(&$value, $keyName = null) {
		...
		$value = lessc_old::compressList($values, ', ');
		return true;
	}
	...
	protected function openString($end, &$out, $nestingOpen=null, $rejectStrs = null) {
		...
		$stop = array("'", '"', "@{", $end);
		$stop = array_map(array("lessc_old", "preg_quote"), $stop);
	...
	protected function literal($what, $eatWhitespace = null) {
		...
		if (!isset(self::$literalCache[$what])) {
			self::$literalCache[$what] = lessc_old::preg_quote($what);
		}
	...
	protected function to($what, &$out, $until = false, $allowNewline = false) {
		...
		if (!$this->match('('.$validChars.'*?)'.lessc_old::preg_quote($what), $m, !$until)) return false;
...
class lessc_formatter_classic_old {
	public $indentChar = "  ";
	...
	public function block($block) {
		...
		if (!empty($block->selectors)) {
			...
			if($block->comments){
				echo implode($this->break . $inner,$block->comments).$this->break;
			}
...
class lessc_formatter_compressed_old extends lessc_formatter_classic_old {
...
class lessc_formatter_lessjs_old extends lessc_formatter_classic_old {
	public $disableSingle = true;
