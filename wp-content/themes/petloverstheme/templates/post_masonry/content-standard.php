...
<?php if (ct_get_context_option("posts_index_show_image", 1) && has_post_thumbnail($post->ID)): ?>
    <div class="ct-articleBox-media">
        <?php get_template_part('templates/post_masonry/content-featured-image'); ?>
    </div>
<?php endif; ?>
...
<div class="ct-articleBox-titleBox">
    <?php if (ct_get_context_option("posts_index_show_title", 1)): ?>
        <h4>
            <a href="<?php echo(esc_url(get_permalink($post->ID))) ?>"><?php echo esc_html(ct_get_blog_item_title()) ?></a>
        </h4>
    <?php endif; ?>
    <!-- Meta -->
